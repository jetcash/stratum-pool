...
function getBlockTemplate(callback){
    apiInterfaces.rpcDaemon('get_block_template', {reserve_size: 8, wallet_address: config.poolServer.poolAddress}, callback);
}
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    hashArray.data.reverse();

    var hashNum = bignum.fromBuffer(new Buffer(hashArray));
    var hashDiff = diff1.div(hashNum);

    if (hashDiff.ge(blockTemplate.difficulty)){
        apiInterfaces.rpcDaemon('submit_block', { "blocktemplate_blob": shareBuffer.toString('hex') }, function(error, result){
            if (error) {
